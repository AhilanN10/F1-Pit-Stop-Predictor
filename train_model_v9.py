"""
XGBoost Pit Stop Classifier v9 — Optuna HPO (2023 only, 19 features)
=====================================================================
v8 14-feature set + 5 new features:
  ahead_compound, ahead_tire_age, behind_compound, behind_tire_age,
  expected_stint_length

Same Optuna setup as v8: 100 trials, 5-fold stratified CV, AP objective,
TPE sampler + median pruner, scale_pos_weight fixed at 2023 class ratio.

Saves model as pit_model_v9.json.
Saves PR curve as precision_recall_curve_v9.png.
"""
...
# ── Baselines ─────────────────────────────────────────────────────────────────
V4 = {"roc": 0.8267, "ap": 0.1446, "label": "v4 (2023, 12 feat, no weather)"}
V8 = {"roc": 0.8393, "ap": 0.1545, "label": "v8 (2023, 14 feat, Optuna)"}

# ── Data prep ─────────────────────────────────────────────────────────────────
raw = pd.read_csv("f1_pit_data_2021_2023.csv")
df  = raw[raw["season"] == 2023].copy()
print(f"Full dataset : {len(raw):,} rows × {raw.shape[1]} columns")
print(f"2023 filter  : {len(df):,} rows  ({100*len(df)/len(raw):.1f}% of total)\n")
...
FEATURE_COLS = [
    ...
    "track_temp",
    "rainfall",
    # Opponent context (v9 new)
    "ahead_compound",
    "ahead_tire_age",
    "behind_compound",
    "behind_tire_age",
    # Stint context (v9 new)
    "expected_stint_length",
]
...
print(f"Class distribution: {n_neg:,} no-pit  |  {n_pos:,} pit")
print(f"scale_pos_weight   : {SPW}  (actual ratio {n_neg/n_pos:.1f}:1)\n")
...
# ── Run optimisation ──────────────────────────────────────────────────────────
print("=" * 64)
print("  Optuna HPO — 100 trials, 5-fold CV, metric = Average Precision")
print(f"  Features : {len(FEATURE_COLS)}  |  Training rows : {len(X_train):,}")
print("=" * 64)
...
print(f"\n  ✓ Optimisation complete  ({len(study.trials)} trials, {n_pruned} pruned)")
...
print("\n" + "=" * 64)
print(f"  EVALUATION  —  pit_model_v9  ({len(FEATURE_COLS)} feat, 2023 only, Optuna)")
print("=" * 64)
...
# ── Feature importances ───────────────────────────────────────────────────────
NEW_FEATS = {"ahead_compound", "ahead_tire_age",
             "behind_compound", "behind_tire_age", "expected_stint_length"}
importances = pd.Series(
    model.feature_importances_, index=FEATURE_COLS
).sort_values(ascending=False)

print(f"\n  FEATURE IMPORTANCES  (weight, {len(FEATURE_COLS)} features)")
print("  " + "-" * 60)
for rank, (feat, score) in enumerate(importances.items(), 1):
    tag = " ← new" if feat in NEW_FEATS else ""
    bar = "█" * int(score * 300)
    print(f"  {rank:>2}. {feat:<24}  {score:.4f}  {bar}{tag}")
...
print("  " + "-" * 68)
print(f"  {'Metric':<30}  {'v4':>8}  {'v8':>8}  {'v9':>8}  {'v9 vs v8':>12}")
print("  " + "-" * 68)
print(f"  {'ROC-AUC':<30}  {V4['roc']:>8.4f}  {V8['roc']:>8.4f}  {roc_auc:>8.4f}  {_d(roc_auc, V8['roc']):>12}")
print(f"  {'Avg Precision (AP)':<30}  {V4['ap']:>8.4f}  {V8['ap']:>8.4f}  {ap:>8.4f}  {_d(ap, V8['ap']):>12}")
print(f"  {'Best CV AP':<30}  {'N/A':>8}  {'0.1860':>8}  {best_cv_ap:>8.4f}  {_d(best_cv_ap, 0.1860):>12}")
print(f"  {'Lift over baseline':<30}  {'4.7×':>8}  {'5.0×':>8}  {ap/baseline:.1f}×{'':<5}")
print(f"  {'Features':<30}  {'12':>8}  {'14':>8}  {len(FEATURE_COLS):>8}")
print(f"  {'HPO':<30}  {'No':>8}  {'Optuna':>8}  {'Optuna':>8}")
print(f"  {'TP @ t=0.50':<30}  {'84':>8}  {'81':>8}  {tp:>8}  {tp-81:>+12}")
print(f"  {'FN @ t=0.50':<30}  {'38':>8}  {'40':>8}  {fn:>8}  {fn-40:>+12}")
print("  " + "-" * 68)
print()

# ── Save model ────────────────────────────────────────────────────────────────
model.save_model("pit_model_v9.json")
print("  Model saved → pit_model_v9.json  ✓")
...
print(f"\n  AP (v9)={ap:.4f}  vs  AP (v8)={V8['ap']:.4f}  Δ={_d(ap, V8['ap'])}")
...
ax.set_title(
    f"Precision–Recall Curve  ·  XGBoost v9  "
    f"({len(FEATURE_COLS)} feat +opponent +stint)  ·  2023  ·  Optuna",
...
ax.text(0.02, 0.22,
        f"AP  v4=0.1446  v8=0.1545  v9={ap:.4f}  "
        f"|  Best CV AP={best_cv_ap:.4f}  Lift: {ap/baseline:.1f}×",
...
plt.savefig("precision_recall_curve_v9.png", dpi=160,
            bbox_inches="tight", facecolor=DARK_BG)
print("  Plot saved → precision_recall_curve_v9.png  ✓")
